...
function buildReportHTML(settings, records, summary, range, mode) {
  const { dateFrom, dateToExclusive, label } = range;
...
<body>
  ...
  <p class="sub">ช่วงวันที่: <b>${escapeHtml(formatThaiDate(dateFrom))}</b> ถึง <b>${escapeHtml(formatThaiDate(prevDate(dateToExclusive)))}</b> • สร้างเมื่อ: ${escapeHtml(now)}</p>
