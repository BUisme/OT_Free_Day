...
function headerTitle() {
  ...
  if (activeTab === 'summary') return 'สรุป (ช่วงเดือน)';
  if (activeTab === 'list') return 'รายการรายวัน (ช่วงเดือน)';
  return 'ตั้งค่า';
}

function buildRangeBar() {
  const { dateFrom, dateToExclusive } = monthRange(selectedMonth);
  const label = `${formatThaiDate(dateFrom)} – ${formatThaiDate(prevDate(dateToExclusive))}`;
  ...
  return el('div', { class:'card', style:'margin-bottom:12px;' },
    ...
    el('div', { class:'bd' },
      el('div', { class:'inline' },
        el('div', { class:'field' }, el('label', {}, 'เลือกเดือน'), monthInput),
        el('div', { class:'field' },
          el('label', {}, 'หมายเหตุ'),
          el('div', { class:'small' }, 'Export PDF/CSV จะใช้ช่วงเดือนนี้')
        )
      )
    )
  );
...
function currentRange() {
  ...
  return { dateFrom, dateToExclusive, label };
}
