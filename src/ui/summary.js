...
import { formatThaiDate, prevDate, dateKey } from '../lib/time.js';
    ...
    el('div', { class:'bd' },
      ...
      kvGrid([
        ...
        ...(hideMoney ? [] : [
          ...
          ['ค่าอาหารรวม', `${(sum.allowancesMeal||0).toFixed(2)} บาท`],
          ['ค่ากะรวม', `${(sum.allowancesShift||0).toFixed(2)} บาท`],
          ['เพิ่มเองรายวันรวม', `${(sum.allowancesManual||0).toFixed(2)} บาท`],
          ['เพิ่มรายเดือน', `${(sum.allowancesMonthly||0).toFixed(2)} บาท`],
          ['หักรายเดือน', `${(sum.deductionsMonthly||0).toFixed(2)} บาท`],
          ['เพิ่มทั้งหมด', `${sum.allowances.toFixed(2)} บาท`],
          ['หักทั้งหมด', `${sum.deductions.toFixed(2)} บาท`],
...
function topOTDays(records, settings, from, to) {
  ...
  for (const r of records || []) {
    ...
    if (k == null) continue;
    if (fromKey != null && k < fromKey) continue;
    if (toKey != null && k >= toKey) continue;
