...
export function buildSettingsView(store) {
  ...
  const body = el('div', {},
    ...
    card('รอบตัดเงินเดือน/OT', [
      el('div', { class:'small' }, 'เลือกเดือนในหน้าหลัก = “เดือนเริ่มรอบ” (ตัวอย่าง: เลือก ก.พ. แล้วตั้ง 21→20 จะกลายเป็น 21/02 ถึง 20/03)'),
      (() => {
        const preset = (Number(s.salaryCycleStartDay || 1) === 21 && s.salaryPayType === 'fixed' && Number(s.salaryPayDay || 0) === 25)
          ? 'd21'
          : 'm1';
        return selectField('เงินเดือน: รอบตัด + วันจ่าย', [
          ['m1', 'ต้นเดือน → สิ้นเดือน • ออกสิ้นเดือน'],
          ['d21', '21 → 20 • ออกวันที่ 25 ของเดือนถัดไป'],
        ], preset, v => {
          if (v === 'd21') {
      ...
      (() => {
        const preset = (s.otCycleMode === 'sameAsSalary')
          ? 'same'
          : (Number(s.otCycleStartDay || 1) === 21 ? 'd21' : 'm1');
        return selectField('OT: รอบตัด + วันจ่าย', [
          ['same', 'ตามเงินเดือน'],
          ['m1', 'ต้นเดือน → สิ้นเดือน • ออกสิ้นเดือน'],
          ['d21', '21 → 20 • ออกวันที่ 25 ของเดือนถัดไป'],
        ], preset, v => {
